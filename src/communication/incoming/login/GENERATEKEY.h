...
void GENERATEKEY(player *player, incoming_message *message) {
    char *cipher_key = ciphering_generate_key();

    outgoing_message *key_data = om_create(1); // "@A"
    sb_add_string(key_data->sb, cipher_key);
    player_send(player, key_data);
    om_cleanup(key_data);

    player->encryption = (encryption *) rc4_create(cipher_key);
