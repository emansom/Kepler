...
void rc4_initialise(encryption *enc, int keyValue) {
    ...
    int keyWindowLength = (sizeof(keyWindow)/ sizeof(keyWindow[0]));

    int keyOffset = keyValue % keyWindowLength;//strlen(keyWindow);
    int tGiven = keyValue;
    int tOwn = 0;

    int *w = malloc(sizeof(int*) * keyLength);
    memset(w, 0, sizeof(w));

    for (int a = 0; a < keyLength; a++) {
        tOwn = keyWindow[abs((keyOffset + a) % keyWindowLength)];//strlen(keyWindow))];
        w[a] = abs(tGiven ^ tOwn);
        if (a == 31)
            tGiven = keyValue;
        else
    ...
    for (int a = 0; a < 256; a++) {
        u = (int)((u + enc->table[a] + enc->key[a]) % 256);
        t = enc->table[a];
        enc->table[a] = enc->table[u];
        enc->table[u] = t;
    }
}
...
char *rc4_encipher(encryption *enc, char *input) {
    ...
    for (int a = 0; a < strlen(input); a++) {
        enc->i = (enc->i + 1) % 256;
        enc->j = (enc->j + enc->table[enc->i]) % 256;
        t = enc->table[enc->i];
        enc->table[enc->i] = enc->table[enc->j];
        enc->table[enc->j] = t;
    }
...
char *rc4_decipher(encryption *enc, char *s) {
    stringbuilder *sb = sb_create();

    int t = 0;
    int k = 0;

    for (int a = 0; a < strlen(s); a += 2) {
        enc->i = (enc->i + 1) % 256;
        enc->j = (enc->j + enc->table[enc->i]) % 256;
        t = enc->table[enc->i];
        enc->table[enc->i] = enc->table[enc->j];
        ...
        char sub[200];
        substring(s, sub, (size_t) a, (size_t) (a + 2) - a);

        k = enc->table[(enc->table[enc->i] + enc->table[enc->j]) % 256];
        t = (int) strtol(sub, NULL, 16);

        sb_add_char(sb, (char)(t ^ k));
    }

    char *return_str = strdup(sb->data);
    sb_cleanup(sb);
...
int rc4_decode_key(char *key) {
    ...
    substring(key, new_table, 0, strlen(key) / 2);
    substring(key, new_key, strlen(key) / 2, strlen(key));

    int checksum = 0;

    for (int i = 0; i < strlen(new_table); i++) {
        char sub[100];
        substring(new_key, sub, (size_t) i, 1);

        char* ptr;
        int offset;

        ptr = strchr(new_table, sub[0]);

        if (ptr == NULL) {
            offset = -1;
        } else {
            offset = (int) (ptr - new_table);
        }
