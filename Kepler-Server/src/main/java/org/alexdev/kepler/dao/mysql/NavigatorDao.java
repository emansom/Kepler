...
public class NavigatorDao {
    ...
    public static List<Room> getRecentRooms(int limit, int categoryId) {
        ...
        try {
            sqlConnection = Storage.getStorage().getConnection();
            preparedStatement = Storage.getStorage().prepare("SELECT * FROM rooms LEFT JOIN users ON rooms.owner_id = users.id WHERE category = ? AND owner_id > 0 ORDER BY rooms.id ASC,visitors_now DESC LIMIT ? ", sqlConnection);
